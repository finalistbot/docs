...
Some hosts ask for files with the entry — a roster screenshot, a rank proof, whatever they
say they need. If the scrim asks for them you'll see the upload on the registration form
with the host's note above it, and if it **requires** them, an entry with nothing attached
is refused. Up to 3 images or PDFs, 8 MB each, visible only to that organization's staff.

![Attaching files to a registration](./assets/entry-attachments-player.png)
...
## Registering from Discord

Two ways, if the server is [connected](../discord/connect-server):

- Press **Register** on the registration-open announcement. A panel shows exactly who would
  be entered, and one more tap enters them.
- Run `/scrim register <share id>`, or `/scrim registered` to see what you're already in.

Both act as *you*, so only your own team can be entered.
