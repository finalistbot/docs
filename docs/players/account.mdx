...
You can save **more than one account per game** — a main and a smurf, a second region, an
old handle — each with a short label of your choosing.

One of them is **primary**. The primary is what every "their IGN" lookup resolves to:
scrim lineups, filters, results. An alt is used only where somebody explicitly picked it,
which today means a [tournament lineup](../tournaments/entering#saved-game-accounts).

![Saved accounts for one game](./assets/saved-game-accounts.png)

Two rules that matter:

- A captain typing a name for a player who has **no** saved account for that game seeds
  theirs.
- A captain can never **overwrite** an account you already saved. It belongs to you.
...
Push works on the web app on devices where you granted permission, so a room-details reveal
reaches you without Discord open.
...
or removed from a scrim, when you're [promoted off a waitlist](../scrims/registering#the-waitlist),
when a scrim is starting, when room details are published, and when results are declared.

Tournaments add their own: your entry being accepted or rejected, check-in opening, your
lobby's room details being revealed, a match's results being confirmed, and the event moving
to its next stage.
