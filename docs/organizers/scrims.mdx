...
| Entry attachments | Off, optional or required — see [Entry attachments](./entry-attachments). |
...
Once a scrim reaches `completed` or `cancelled` it drops out of the scrims list and into
**Scrims → Archive**, newest first. The archive is scoped to the active organization and
holds both outcomes, so a cancelled scrim stays on the record rather than disappearing.
The summary card above the scrims list shows how many are archived and which one wrapped
most recently.

Players see a narrower version of the same thing: their archive lists **only completed**
scrims, because a cancelled one was never played and carries no lineup or result.
