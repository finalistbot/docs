...
Every scrim has a live chat on its page, and it's usually where the lobby sorts itself out.
Who can post depends on the mode the host set — sometimes everyone, sometimes only slotted
players. See [Scrim chat](./chat).
...
**Scrims → Archive** keeps every scrim that played through to a result, newest first. Open
one to see the final lineup and the declared result. Cancelled scrims are deliberately left
out — nothing was played, so there's nothing to look back at.

## If something goes wrong

Cheating, throwing, abuse in chat, a team that never turned up: [report it](../players/reporting).
Reports go to the organization that hosted the scrim.
